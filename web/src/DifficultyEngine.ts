// DifficultyEngine.ts - Sawtooth Difficulty Configuration

interface LevelConfig {
  matchDensity: number;
  decoyRatio: number;
  chainLength: number;
  averageBranching: number;
  scanDistance: string;
  isolatedCells: number;
  addRowDifficulty: number;
  targetSolveTime: number;
  expectedAddRow: number;
  difficultyScore: number;
  seed: number;
  minGap?: number;
  maxGap?: number;
  trueDecoyRatio?: number;
  frictionFactor?: number;
}

var LEVEL_CONFIG: LevelConfig[] = [
  {
    matchDensity: 0.70, frictionFactor: 0.00, trueDecoyRatio: 0.00,
    minGap: 1,  maxGap: 3,
    chainLength: 2, averageBranching: 1.2,
    scanDistance: "1-3", isolatedCells: 0,
    addRowDifficulty: 1, targetSolveTime: 45,
    expectedAddRow: 1, difficultyScore: 1,
    seed: 1000003,
    decoyRatio: 0.00
  },
  {
    matchDensity: 0.60, frictionFactor: 0.12, trueDecoyRatio: 0.08,
    minGap: 2,  maxGap: 4,
    chainLength: 3, averageBranching: 1.5,
    scanDistance: "2-4", isolatedCells: 1,
    addRowDifficulty: 2, targetSolveTime: 65,
    expectedAddRow: 1.5, difficultyScore: 2,
    seed: 1000033,
    decoyRatio: 0.08
  },
  {
    matchDensity: 0.50, frictionFactor: 0.25, trueDecoyRatio: 0.18,
    minGap: 3,  maxGap: 6,
    chainLength: 4, averageBranching: 1.8,
    scanDistance: "3-6", isolatedCells: 2,
    addRowDifficulty: 3, targetSolveTime: 90,
    expectedAddRow: 2.5, difficultyScore: 3,
    seed: 1000037,
    decoyRatio: 0.18
  },
  {
    matchDensity: 0.45, frictionFactor: 0.38, trueDecoyRatio: 0.28,
    minGap: 4,  maxGap: 10,
    chainLength: 5, averageBranching: 2.0,
    scanDistance: "4-10", isolatedCells: 3,
    addRowDifficulty: 4, targetSolveTime: 120,
    expectedAddRow: 2.5, difficultyScore: 4,
    seed: 1000039,
    decoyRatio: 0.28
  },
  {
    matchDensity: 0.40, frictionFactor: 0.50, trueDecoyRatio: 0.42,
    minGap: 6,  maxGap: 14,
    chainLength: 6, averageBranching: 2.2,
    scanDistance: "6-14", isolatedCells: 4,
    addRowDifficulty: 5, targetSolveTime: 150,
    expectedAddRow: 3, difficultyScore: 6,
    seed: 1000081,
    decoyRatio: 0.42
  },
  {
    matchDensity: 0.50, frictionFactor: 0.25, trueDecoyRatio: 0.18,
    minGap: 3,  maxGap: 6,
    chainLength: 4, averageBranching: 1.8,
    scanDistance: "3-6", isolatedCells: 2,
    addRowDifficulty: 3, targetSolveTime: 90,
    expectedAddRow: 3, difficultyScore: 3,
    seed: 1000099,
    decoyRatio: 0.18
  },
  {
    matchDensity: 0.45, frictionFactor: 0.40, trueDecoyRatio: 0.30,
    minGap: 4,  maxGap: 10,
    chainLength: 5, averageBranching: 2.0,
    scanDistance: "4-10", isolatedCells: 3,
    addRowDifficulty: 6, targetSolveTime: 120,
    expectedAddRow: 3, difficultyScore: 6,
    seed: 1000117,
    decoyRatio: 0.36
  },
  {
    matchDensity: 0.40, frictionFactor: 0.52, trueDecoyRatio: 0.44,
    minGap: 6,  maxGap: 14,
    chainLength: 6, averageBranching: 2.2,
    scanDistance: "6-14", isolatedCells: 4,
    addRowDifficulty: 7, targetSolveTime: 150,
    expectedAddRow: 3.5, difficultyScore: 7,
    seed: 1000121,
    decoyRatio: 0.48
  },
  {
    matchDensity: 0.35, frictionFactor: 0.64, trueDecoyRatio: 0.56,
    minGap: 8,  maxGap: 18,
    chainLength: 7, averageBranching: 2.4,
    scanDistance: "8-18", isolatedCells: 5,
    addRowDifficulty: 8, targetSolveTime: 180,
    expectedAddRow: 4, difficultyScore: 8,
    seed: 1000133,
    decoyRatio: 0.58
  },
  {
    matchDensity: 0.30, frictionFactor: 0.75, trueDecoyRatio: 0.65,
    minGap: 9,  maxGap: 22,
    chainLength: 8, averageBranching: 2.6,
    scanDistance: "9-22", isolatedCells: 6,
    addRowDifficulty: 10, targetSolveTime: 210,
    expectedAddRow: 4.5, difficultyScore: 10,
    seed: 1000151,
    decoyRatio: 0.68
  },
  {
    matchDensity: 0.50, frictionFactor: 0.25, trueDecoyRatio: 0.18,
    minGap: 3,  maxGap: 6,
    chainLength: 4, averageBranching: 1.8,
    scanDistance: "3-6", isolatedCells: 2,
    addRowDifficulty: 3, targetSolveTime: 90,
    expectedAddRow: 3, difficultyScore: 3,
    seed: 1000159,
    decoyRatio: 0.18
  }
];

function getLevelConfig(lvlIndex: number): LevelConfig {
  if (lvlIndex < LEVEL_CONFIG.length) {
    return LEVEL_CONFIG[lvlIndex];
  }
  // Wrap index back to level configs 1 to 10 (index 1 to 10), repeating the sawtooth difficulty curve!
  var idx = 1 + ((lvlIndex - 1) % (LEVEL_CONFIG.length - 1));
  var base = LEVEL_CONFIG[idx];
  
  return {
    matchDensity: base.matchDensity,
    decoyRatio: base.decoyRatio,
    chainLength: base.chainLength,
    averageBranching: base.averageBranching,
    scanDistance: base.scanDistance,
    isolatedCells: base.isolatedCells,
    addRowDifficulty: base.addRowDifficulty,
    targetSolveTime: base.targetSolveTime,
    expectedAddRow: base.expectedAddRow,
    difficultyScore: base.difficultyScore,
    seed: (base.seed + lvlIndex * 997) >>> 0, // dynamic unique seed
    minGap: base.minGap,
    maxGap: base.maxGap,
    trueDecoyRatio: base.trueDecoyRatio,
    frictionFactor: base.frictionFactor
  };
}

function isReliefLevel(lvlIndex: number): boolean {
  if (lvlIndex < LEVEL_CONFIG.length) {
    return lvlIndex === 5 || lvlIndex === 10;
  }
  var idx = 1 + ((lvlIndex - 1) % (LEVEL_CONFIG.length - 1));
  return idx === 5 || idx === 10;
}

// For compatibility with legacy config naming
var LCFG = LEVEL_CONFIG.map(function(c) {
  return {
    md: c.matchDensity,
    dr: c.decoyRatio,
    sv: 1 - c.decoyRatio, // SV compatibility
    seed: c.seed
  };
});

var RELIEF_IDX: { [key: number]: number } = { 5: 1, 10: 1 };

// Global exports
(globalThis as any).LEVEL_CONFIG = LEVEL_CONFIG;
(globalThis as any).getLevelConfig = getLevelConfig;
(globalThis as any).isReliefLevel = isReliefLevel;
(globalThis as any).LCFG = LCFG;
(globalThis as any).RELIEF_IDX = RELIEF_IDX;
