...
interface LevelConfig {
  // Core density / friction
  matchDensity: number;
  decoyRatio: number;
  frictionFactor: number;       // 0=all same-value pairs, 1=all sum-10 pairs

  // NEW: Structural difficulty controls
  targetImmediateRatio: number; // Fraction of cells in an *immediately* matchable pair at game start
  targetAvgGap: number;         // Target average index-gap between paired cells
  buriedPairCount: number;      // # of pairs intentionally placed with active blockers between them
  effectiveDecoyRatio: number;  // Fraction of filler cells that are TRUE decoys (no match possible)
  strictMatchDensityTolerance: number; // Acceptance window for match density check (tight = harder board)
...
var RELIEF_IDX: { [key: number]: number } = { 5: 1, 10: 1 };

// Global exports
(globalThis as any).LEVEL_CONFIG = LEVEL_CONFIG;
(globalThis as any).getLevelConfig = getLevelConfig;
(globalThis as any).isReliefLevel = isReliefLevel;
(globalThis as any).LCFG = LCFG;
(globalThis as any).RELIEF_IDX = RELIEF_IDX;
