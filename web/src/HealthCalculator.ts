...
interface HealthReport {
  ...
  orphanCells: number[];
  isolatedRows: number;
  averageScanDistance: number;
  futureChainPotential: number;
  deadEndRisk: number;
}

// Find orphan cells (active cells with no matching values anywhere else on the board)
function getOrphanCells(board: Cell[]): number[] {
  var activeIndices: number[] = [];
  for (var i = 0; i < board.length; i++) {
    if (!board[i].m) activeIndices.push(i);
  }
...
function getIsolatedRowsCount(board: Cell[]): number {
...
function getAverageScanDistance(board: Cell[], matches: [number, number][]): number {
...
function getFutureChainPotential(board: Cell[], matches: [number, number][]): number {
  if (matches.length === 0) return 0;
  var newMatchesSum = 0;
...
function getDeadEndRisk(board: Cell[], matches: [number, number][]): number {
...
function calculateBoardHealth(board: Cell[]): HealthReport {
...
// Global exports
(globalThis as any).getOrphanCells = getOrphanCells;
(globalThis as any).getIsolatedRowsCount = getIsolatedRowsCount;
(globalThis as any).calculateBoardHealth = calculateBoardHealth;
