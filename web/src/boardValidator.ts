...
function isBoardSolvableWithAddRowsLimit(board: Cell[], cfg: LevelConfig, maxAR: number): boolean {
  ...
  var solver = (globalThis as any).solveBoard || solveBoard;
...
function countRequiredAddRows(board: Cell[], cfg: LevelConfig, maxAR: number): number {
  var sim = board.map(function(c){ return {v: c.v, m: c.m}; });
  var arUsed = 0;
  var dryP = 0;
  var matchesSinceLastAdd = 0;
  ...
  while (true) {
    var active = 0;
    for (var i = 0; i < sim.length; i++) if (!sim[i].m) active++;
    if (active === 0) return arUsed; // board cleared, return AR count
...
var MIN_REQUIRED_ADD_ROWS: number[] = [
...
(globalThis as any).countRequiredAddRows = countRequiredAddRows;
