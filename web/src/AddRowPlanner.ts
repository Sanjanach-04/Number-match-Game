...
function selectWeighted(candidates: { val: number; weight: number }[], rng: any): number {
  var totalWeight = 0;
  for (var i = 0; i < candidates.length; i++) {
    totalWeight += candidates[i].weight;
  }
  if (totalWeight === 0) return rng.range(1, 9);
  var rand = rng.int(totalWeight);
  var sum = 0;
  for (var i = 0; i < candidates.length; i++) {
    sum += candidates[i].weight;
    if (rand < sum) return candidates[i].val;
  }
...
function executeAddRow(board: Cell[], cfg: LevelConfig, dryPresses: number): { board: Cell[]; val: number; wasRescue: boolean } {
