...
function _greedyRun(board, pickFn) {
...
function isBoardSolvable(board) {
    ...
    if (_greedyRun(board, _pickFirst) <= target)
        return true;
    if (_greedyRun(board, _pickLast) <= target)
        return true;
    if (_greedyRun(board, _pickShortest) <= target)
        return true;
    if (_greedyRun(board, _pickLongest) <= target)
        return true;
    if (_greedyRun(board, function (ms) { return _pickRarest(ms, board); }) <= target)
        return true;
    return false;
}
...
function solveBoard(board) {
    ...
    var strategies = [_pickFirst, _pickLast, _pickShortest, _pickLongest,
        function (ms) { return _pickRarest(ms, board); }];
...
// Global exports
globalThis.isBoardSolvable = isBoardSolvable;
globalThis.solveBoard = solveBoard;
