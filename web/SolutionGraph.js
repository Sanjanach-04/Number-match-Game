...
function isBitSet(mask, i) {
    return (mask[Math.floor(i / 32)] & (1 << (i % 32))) !== 0;
}
// Check matching rules under a specific bitmask state
function canMatchWithMask(board, mask, a, b) {
    if (a === b || isBitSet(mask, a) || isBitSet(mask, b))
        return false;
    ...
    if (Math.abs(dr) === Math.abs(dc) && dr !== 0) {
        ...
        while (r !== rb || c !== cb) {
            var idx = r * COLS + c;
            if (idx >= 0 && idx < board.length && !isBitSet(mask, idx)) {
                ok = false;
                break;
            }
            r += sr;
...
function solveBoard(board) {
    ...
    function dfs(mask, activeCount) {
        ...
        var matches = [];
        for (var i = 0; i < len; i++) {
            if ((mask[Math.floor(i / 32)] & (1 << (i % 32))) !== 0)
                continue;
            for (var j = i + 1; j < len; j++) {
                if ((mask[Math.floor(j / 32)] & (1 << (j % 32))) !== 0)
                    continue;
